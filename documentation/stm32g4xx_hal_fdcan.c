...
#define SRAMCAN_FLESA                                                                                                  \
    ((uint32_t)(SRAMCAN_FLSSA + (SRAMCAN_FLS_NBR * SRAMCAN_FLS_SIZE)))                   /* Filter List Extended Start \
                                                                                            Address                  */
...
#define SRAMCAN_TEFSA                                                                         \
    ((uint32_t)(SRAMCAN_RF1SA + (SRAMCAN_RF1_NBR * SRAMCAN_RF1_SIZE))) /* Tx Event FIFO Start \
                                                                          Address */
#define SRAMCAN_TFQSA                                                                                          \
    ((uint32_t)(SRAMCAN_TEFSA + (SRAMCAN_TEF_NBR * SRAMCAN_TEF_SIZE)))                  /* Tx FIFO/Queue Start \
                                                                                           Address                  */
...
HAL_StatusTypeDef HAL_FDCAN_Init(FDCAN_HandleTypeDef* hfdcan) {
    ...
    hfdcan->Instance->NBTP =
        ((((uint32_t)hfdcan->Init.NominalSyncJumpWidth - 1U) << FDCAN_NBTP_NSJW_Pos) |
         (((uint32_t)hfdcan->Init.NominalTimeSeg1 - 1U) << FDCAN_NBTP_NTSEG1_Pos) | (((uint32_t)hfdcan->Init.NominalTimeSeg2 - 1U) << FDCAN_NBTP_NTSEG2_Pos) |
         (((uint32_t)hfdcan->Init.NominalPrescaler - 1U) << FDCAN_NBTP_NBRP_Pos));

    /* If FD operation with BRS is selected, set the data bit timing register */
    if(hfdcan->Init.FrameFormat == FDCAN_FRAME_FD_BRS) {
        hfdcan->Instance->DBTP =
            ((((uint32_t)hfdcan->Init.DataSyncJumpWidth - 1U) << FDCAN_DBTP_DSJW_Pos) | (((uint32_t)hfdcan->Init.DataTimeSeg1 - 1U) << FDCAN_DBTP_DTSEG1_Pos) |
             (((uint32_t)hfdcan->Init.DataTimeSeg2 - 1U) << FDCAN_DBTP_DTSEG2_Pos) | (((uint32_t)hfdcan->Init.DataPrescaler - 1U) << FDCAN_DBTP_DBRP_Pos));
    }
...
HAL_StatusTypeDef HAL_FDCAN_ConfigFilter(FDCAN_HandleTypeDef* hfdcan, const FDCAN_FilterTypeDef* sFilterConfig) {
    ...
    if((state == HAL_FDCAN_STATE_READY) || (state == HAL_FDCAN_STATE_BUSY)) {
        ...
        if(sFilterConfig->IdType == FDCAN_STANDARD_ID) {
            ...
            FilterElementW1 =
                ((sFilterConfig->FilterType << 30U) | (sFilterConfig->FilterConfig << 27U) | (sFilterConfig->FilterID1 << 16U) | sFilterConfig->FilterID2);
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_FDCAN_ConfigGlobalFilter(FDCAN_HandleTypeDef* hfdcan, uint32_t NonMatchingStd, uint32_t NonMatchingExt, uint32_t RejectRemoteStd,
    ...
    if(hfdcan->State == HAL_FDCAN_STATE_READY) {
        ...
        MODIFY_REG(hfdcan->Instance->RXGFC, (FDCAN_RXGFC_ANFS | FDCAN_RXGFC_ANFE | FDCAN_RXGFC_RRFS | FDCAN_RXGFC_RRFE),
                   ((NonMatchingStd << FDCAN_RXGFC_ANFS_Pos) | (NonMatchingExt << FDCAN_RXGFC_ANFE_Pos) | (RejectRemoteStd << FDCAN_RXGFC_RRFS_Pos) |
...
HAL_StatusTypeDef HAL_FDCAN_GetRxMessage(FDCAN_HandleTypeDef* hfdcan, uint32_t RxLocation, FDCAN_RxHeaderTypeDef* pRxHeader, uint8_t* pRxData) {
    ...
    /* Check function parameters */
    //  assert_param(IS_FDCAN_RX_FIFO(RxLocation)); // TODO
...
static void FDCAN_CopyMessageToRAM(const FDCAN_HandleTypeDef* hfdcan, const FDCAN_TxHeaderTypeDef* pTxHeader, const uint8_t* pTxData, uint32_t BufferIndex) {
    ...
    TxElementW2 =
        ((pTxHeader->MessageMarker << 24U) | pTxHeader->TxEventFifoControl | pTxHeader->FDFormat | pTxHeader->BitRateSwitch | (pTxHeader->DataLength << 16U));
